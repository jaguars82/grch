...
namespace app\components\import\Krays;
...
/**
 * Service for importing flat's data from xml-feed for developer "Крайс"
 */
        ...
        foreach ($data->complex as $complex) {
			...
			foreach ($complex->buildings->building as $building) {
				...
				foreach ($building->flats->flat as $flat) {
					$area = str_replace(',', '.', $flat->area);
					$unitPrice = ($flat->area != 0) ? ((int)$flat->price / (float)$area) : 0;
                    ...
                    if((int)$flat->room === 0 && (float)$area > 5) {
                        $room = 1;
                        $studio = 1;
                    } else {
                        $room = (int)$flat->room;
