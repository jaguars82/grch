...
<div class="commercial-offer">
    ...
    <div class="white-bg">
        <div class="row">
            <?php if(!is_null($flat->layout)): ?>
                <div class="col-xs-12 col-sm-5 layout-wrap">
        ...
        <?php if($hasCoords): ?>  
            <div class="address-inline-map">
                <?php if(isset($isPlacemarkImage) && $hasCoords): ?>
                    <?= Html::img("https://static-maps.yandex.ru/1.x/?ll={$flat->newbuildingComplex->latitude},{$flat->newbuildingComplex->longitude}&size=595,379&z=16&l=map&pt={$flat->newbuildingComplex->latitude},{$flat->newbuildingComplex->longitude},pm2vvm") ?>
                <?php else: ?>
                    <?= Placemark::widget([
    ...
    <?php if(!is_null($flat->newbuildingComplex->master_plan)): ?>
        <div style="background-image: url(<?= "$imagePath/uploads/{$flat->newbuildingComplex->master_plan}" ?>)" class="master-plan"></div>
    <?php endif; ?>
